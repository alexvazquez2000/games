...
public class Solitaire {
		...
		for (Move move : moves) {
			...
			if (fromIndex >= 0) {
				...
				if (toCol.isValidMove(card) > 0) {
					...
					madeAMove = true;
					System.out.println("Move col[" + move.getFromCol() + "]" + fromIndex + " -> to col["
							+ move.getToCol() + "]=" + move.getCard().getLongName() + "(" + move.getWeight() + ")");
	...
	private List<Move> findMovesAllowed() {
		...
		for (int i = 0; i < 10; i++) {
			...
			// TODO: The allowedCut should have the index of the cell. If index is 0 then it
			// allows to have an empty column
	...
	private boolean dealNextRow() {
		...
		// TODO: After dealing check if any of the new cards in the columns make a
		// fullChain
		return true;
	}
